...
**Your path is the MCP server `vteam-brain-cerberus`**, which talks to the running
...
```
ToolSearch  select:mcp__vteam-brain-cerberus__whoami,mcp__vteam-brain-cerberus__get_page,mcp__vteam-brain-cerberus__query,mcp__vteam-brain-cerberus__list_pages

mcp__vteam-brain-cerberus__list_pages {}                              # what you hold
mcp__vteam-brain-cerberus__get_page   {"slug":"orientation-notes"}    # your ramp notes
mcp__vteam-brain-cerberus__query      {"query":"<terms>"}             # your store + default
```
...
sentences and only you can tell them apart. `mcp__vteam-brain-cerberus__whoami`
...
grants `read` on `cerberus` and `default` and nothing else: naming another
...
export GBRAIN_HOME="$HOME/.v-team/clients/cerberus"
